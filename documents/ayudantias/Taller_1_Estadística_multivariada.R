...
# Material de ayudantía: Taller n°1. 
# Manejo de bases de datos (abrir, filtrar, recodificar, descriptivos básicos, creación de índices y correlación de Pearson)
...
#Estavkece tu propio directorio con Ctrl + Shift + H

##### ------------------ 2° Abrir y Visualizar ---------------- #####
install.packages("haven") #Instalamos paquete para importar SPSS y Stata
library(haven) #Llamamos al paquete haven en la librería

#Dos formas de abrir la base 
#Abre la base desde el directorio que hemos elegido si tenemos la base guardada allí
base <- read_spss("ELSOC.sav") #Formato .sav abre la base de datos formato SPSS.

#Abrir la base directamente de una página
...
install.packages("dplyr") #Este paquete ayuda a manejar bases de datos
library(dplyr) #Lo llamamos el paquete desde la libreria
...
ELSOC <- na.omit(ELSOC)# Elimina todas las filas o casos que contengan algun valor perdido 


#Opción 2 (utilizando comando filter de la librería dplyr)
ELSOC$Conf.Vecinos <- as.numeric(ELSOC$Conf.Vecinos) #convertimos la variable en un vector numerico

# Pedimos que nos filtre por valores > 0 , filtrando los -888 y -999
...
#Matriz de pearson
ELSOC <- select(ELSOC, -Conf.Vecinosrec) #Eliminamos Variable Conf.Vecinosrec para calcular matriz de pearson
ELSOC.cor <- cor(ELSOC, method = "pearson") #method = tipo de correlación: pearson, spearman, kendall
View(ELSOC.cor)
ELSOC.cor <- round(cor(ELSOC), 2) #Comando round para redondear. En este caso le indicamos redondear a 2 decimales
View(ELSOC.cor)


#Para evaluar la posible existencia de esta correlacion en el universo, debemos observar el p de significacion.
#Este es un ejemplo de como calcular el valor p de una correlacion.
#En este caso, se correlaciona la Variable Ac.Barrio.part con la variable Ac.Barrio.ident
#cor.test(x,y)
cor.test(ELSOC$Ac.Barrio.ident, ELSOC$Ac.Barrio.part)